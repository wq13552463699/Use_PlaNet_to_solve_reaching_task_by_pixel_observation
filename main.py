...
parser.add_argument('--id', type=str, default='s2r', help='Experiment ID')
parser.add_argument('--seed', type=int, default=1, metavar='S', help='Random seed')
parser.add_argument('--disable-cuda', action='store_true', help='Disable CUDA')
parser.add_argument('--max-episode-length', type=int, default=300, metavar='T', help='Max episode length')
parser.add_argument('--experience-size', type=int, default=50000, metavar='D', help='Experience replay size')  # Original implementation has an unlimited buffer size, but 1 million is the max experience collected anyway
...
################################ This parameter determine the memory size to use#################################################
# parser.add_argument('--hidden-size', type=int, default=1000, metavar='H', help='Hidden size')
parser.add_argument('--hidden-size', type=int, default=200, metavar='H', help='Hidden size')

parser.add_argument('--belief-size', type=int, default=200, metavar='H', help='Belief/hidden size')
# parser.add_argument('--belief-size', type=int, default=1000, metavar='H', help='Belief/hidden size')

parser.add_argument('--state-size', type=int, default=50, metavar='Z', help='State/latent size')
# parser.add_argument('--state-size', type=int, default=500, metavar='Z', help='State/latent size')

parser.add_argument('--seed-episodes', type=int, default=2, metavar='S', help='Seed episodes')
parser.add_argument('--collect-interval', type=int, default=30, metavar='C', help='Collect interval')
parser.add_argument('--batch-size', type=int, default=40, metavar='B', help='Batch size')
parser.add_argument('--chunk-size', type=int, default=40, metavar='L', help='Chunk size')
##############################################################################################################################
...
parser.add_argument('--checkpoint-interval', type=int, default=1, metavar='I', help='Checkpoint interval (episodes)')
...
args.models = '/home/qiang/UR5-v0/results/1cameras-small/models_565.pth'
# args.experience_replay = '/home/qiang/UR5-v0/results/rep_1r_1p/experience.pth'
# Setup
...
metrics = {'steps': [], 'episodes': [], 'train_rewards': [], 'observation_loss': [], 'reward_loss': [], 'kl_loss': []}

args.symbolic_env = False
# Initialise training environment and experience replay memory
env = GymEnv(args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth)
# env = rep_env(args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth)
if args.experience_replay is not '' and os.path.exists(args.experience_replay):
  ...
  for s in range(1):
    ...
    while not done:
      ...
      #TODO: This place need to be re-modified, since it has possibilitied cannot get enough experience for chunck
...
if args.models is not '' and os.path.exists(args.models):
  print('Model loaded,Model loaded,Model loaded,Model loaded,Model loaded')
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes, initial=metrics['episodes'][-1] + 1):
  ...
  # Checkpoint models
  args.checkpoint_experience = True
